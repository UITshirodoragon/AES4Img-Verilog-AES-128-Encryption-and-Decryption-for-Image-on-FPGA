...
- Có 3 top-level để chọn theo mục tiêu: ROM preload, SRAM preload, hoặc UART live demo.
...
legacy/          Source cũ để đối chiếu
legacy_uart/     UART thử nghiệm cũ để tham khảo
```
...
|---|---|---|
| `rtl/top_uart.v` | Demo thật với PC gửi ảnh qua UART | Khuyến nghị. Quartus entity là `top_uart`. |
| `rtl/top_preloaded.v` | Ảnh gốc đã được nạp sẵn vào SRAM bằng công cụ khác | Bỏ qua UART wait bằng thiết kế preloaded. |
| `rtl/top.v` | Mô phỏng hoặc FPGA có đủ internal RAM cho ROM ảnh | Dùng `image_rom_320x240_rgb565.v` đọc file HEX. |

## Chạy demo UART trên board

1. Mở `quartus/AES_128_v2_1_uart.qpf` bằng Quartus II.
2. Kiểm tra `TOP_LEVEL_ENTITY` trong QSF đang là `top_uart`.
